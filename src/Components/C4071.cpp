...
/*
** The component 4071 is composed of 14 pins. It has 4 OR gates
...
C4071::C4071(const std::string &name) : AComponent(name, "chipset") {
  ...
  // Create the pins of the chipset 4071 and set them.
...
*/
bool            C4071::doesComponentTypeMatch(AComponent &component, size_t pin) {
...
** Output to an Input)
*/
bool            C4071::doesPinsTypesMatch(size_t pin_num_this, size_t pin_num_target) {
...
void    C4071::SetLink(size_t pin_num_this,
  ...
  if (!pinIndexIsValid(pin_num_this))
    throw Error("[ C4071 " + this->_name + " | LINK] : Invalid pin selected ("
  ...
  if (this == &component && !doesPinsTypesMatch(pin_num_this, pin_num_target))
    throw Error("[ C4071 " + this->_name + " | LINK] : Impossible to link the pin "
  ...
  // Check if the component type match with the type expected by the pin.
    ...
    throw Error("[ C4071 " + this->_name + " | LINK] : Component type expected by the pin "
                + std::to_string((int)pin_num_target) + " doesn't correspond with the type of the component '"
                + (*dynamic_cast<AComponent *>(&component)).getName() + "'.");
