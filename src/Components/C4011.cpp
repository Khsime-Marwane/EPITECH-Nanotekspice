...
/*
** The component 4011 is composed of 14 pins. It has 4 OR gates
** which works for each of them with two inputs and one output.
*/
C4011::C4011(const std::string &name) : AComponent(name, "chipset") {
  ...
  // Create the pins of the chipset 4011 and set them.
  ...
  if (pinIndexIsValid(pin_num_this))
    {
      ...
      if (this->gateLinks.find(pin_num_this) != this->gateLinks.end()) {
          ...
          // Call the door NAND with v1 and v2 as parameters.
          this->pins[pin_num_this - 1].state = this->gate.compute("NAND", v1, v2);
      }
...
*/
bool            C4011::pinIndexIsValid(size_t pin_num_this) {
...
*/
bool            C4011::doesComponentTypeMatch(AComponent &component,
                                              size_t pin_num_this,
...
** Output to an Input)
*/
bool            C4011::doesPinsTypesMatch(size_t pin_num_this, size_t pin_num_target) {
...
*/
void    C4011::SetLink(size_t pin_num_this,
                       nts::IComponent &component,
                       size_t pin_num_target) {
  // Check if the index (pin_num_this) is valid.
  if (!pinIndexIsValid(pin_num_this))
    throw Error("[ C4011 " + this->_name + " | LINK] : Invalid pin selected ("
  ...
  if (this == &component) {
    if (!doesPinsTypesMatch(pin_num_this, pin_num_target))
      throw Error("[ C4011 " + this->_name + " | LINK] : Impossible to link the pin "
  ...
  // Check if the component type match with the type expected by the pin.
    ...
    if (!doesComponentTypeMatch(*dynamic_cast<AComponent *>(&component), pin_num_this, pin_num_target))
      throw Error("[ C4011 " + this->_name + " | LINK] : Component type expected by the pin "
                  + std::to_string((int)pin_num_target) + " doesn't correspond with the type of the component '"
                  + (*dynamic_cast<AComponent *>(&component)).getName() + "'.");
